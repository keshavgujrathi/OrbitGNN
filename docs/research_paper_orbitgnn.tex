...
\begin{table*}[t]
\centering
\caption{Full Model Comparison on Real TLE Benchmark (Per-Satellite, $N=1305$, 58 Positives, 3 Seeds)}
\label{tab:main}
...
\midrule
\orbitgnn\ v1 (orig.)       & Causal, self-sup. & 0.591\tiny{$\pm$.002} & 0.169\tiny{$\pm$.001} & 0.234\tiny{$\pm$.018} & 0.898 & 0.594 & 0.207\tiny{$\pm$.025} & 0.195 \\
\textbf{\orbitgnn\ opt.\ (ours)} & Causal, self-sup. & \textbf{0.588}\tiny{$\pm$.009} & \textbf{0.176}\tiny{$\pm$.002} & \textbf{0.305}\tiny{$\pm$.007} & 0.898 & \textbf{0.637} & \textbf{0.302}\tiny{$\pm$.004} & \textbf{0.288}\\
\bottomrule
\multicolumn{9}{l}{\footnotesize Bold: best self-supervised/unsupervised causal method. Supervised methods require labels (unavailable for non-coop.\ satellites).}\\
\multicolumn{9}{l}{\footnotesize Opt.\ config: $W{=}4$, $k{=}2$, $w_\text{self}{=}2.0$, $w_\text{peer}{=}0.5$, $w_\text{unc}{=}0.0$; threshold on val set; 3-seed mean$\pm$std.}
\end{tabular}
\end{table*}
...
\orbitgnn\ optimised is the \textbf{best self-supervised causal method}:
ROC-AUC $= 0.588$ ($+0.177$ vs.\ ResidMag), F1 $= 0.305$ ($+0.170$ vs.\ LSTM-AE),
MCC $= 0.302$ ($+0.215$ vs.\ LSTM-AE). Optimisation over the original ($W{=}8$) yields
$\Delta$F1 $= +0.071$ and $\Delta$MCC $= +0.095$ (3-seed mean). It remains the
\textbf{only} self-supervised method providing peer-satellite context and $\Delta V$ estimates.
...
\begin{figure}[h]
  ...
  \caption{Confusion matrices for all 9 methods (per-satellite, test split).
    \orbitgnn\ optimised achieves the best F1 among self-supervised methods (0.305, $W{=}4$, $k{=}2$).
...
\subsection{Hyperparameter Optimisation (Validation Set Only)}
\label{sec:optim}

We conducted a systematic ablation on the \emph{validation set only} (no test-set access)
to identify the best configuration. All threshold selection also uses the validation set.

\textbf{Window size} ($W \in \{4,6,8,10,14\}$, seed~1):
$W{=}4$ achieves the highest validation ROC-AUC ($0.665$), outperforming
$W{=}8$ by $+0.051$. Shorter windows better capture sharp, discrete manoeuvre events.

\textbf{Anomaly score weights.}
A grid search over $(w_\text{self},\, w_\text{peer},\, w_\text{unc}) \in \{0, 0.5, 1, 2\}^3$
reveals that the MC-Dropout uncertainty term is near-random on this dataset
(uncertainty-only ROC-AUC $= 0.494 \approx$ chance). Best weights:
$w_\text{self}{=}2.0$, $w_\text{peer}{=}0.5$, $w_\text{unc}{=}0.0$.

\textbf{Graph connectivity} ($k \in \{1,2,3,4,5\}$, $W{=}4$):
$k{=}2$ achieves val ROC-AUC $= 0.723$ vs.\ $k{=}3$'s $0.702$.

\begin{table}[h]
\centering\caption{Window-Size Ablation (Validation ROC-AUC, Seed 1)}
\label{tab:window}
\setlength\tabcolsep{5pt}
...
$W$ & 4 & 6 & 8 (orig.) & 10 & 14 \\
\midrule
Val ROC & \textbf{0.665} & 0.634 & 0.615 & 0.623 & 0.618 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{fig15_optimization_comparison}
  \caption{Optimisation results: (a)~metric comparison (original vs.\ optimised, 3 seeds);
    (b)~per-seed trajectory. Optimised model shows $+0.071$ F1 and $+0.095$ MCC
    with reduced variance across seeds.}
  \label{fig:optim}
\end{figure}
